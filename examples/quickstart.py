...
``qg.load_panel([...], "2018-01-01", "2024-12-31", market="US")`` for real data.
...
def synthetic_panel(
    ...
    stamps = pd.DatetimeIndex([qg.US.session_close_utc(d) for d in dates])
...
def mean_reversion(view: qg.PanelView) -> pd.DataFrame:
...
def oracle(view: qg.PanelView) -> pd.DataFrame:
...
def main() -> None:
    ...
    arena = qg.Arena(panel, config, perturbation_samples=4)
    arena.add(qg.RuleBasedStrategy(momentum, warmup=126), "momentum_6m")
    arena.add(qg.BatchRuleStrategy(mean_reversion, warmup=5), "mean_reversion_1w")
    arena.add(qg.BatchRuleStrategy(oracle, warmup=1), "oracle_LEAKY")
    result = arena.run()
